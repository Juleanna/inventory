...
class License(models.Model):
    ...
    activations = models.IntegerField(
        default=1,
        verbose_name="Кількість активацій"
    )
    start_date = models.DateField(
        null=True,
        blank=True,
        verbose_name="Початок дії"
    )
    end_date = models.DateField(
        null=True,
        blank=True,
        verbose_name="Кінець дії"
    )
    is_perpetual = models.BooleanField(
        default=False,
        verbose_name="Безстрокова"
    )
    ...
    trial_days = models.IntegerField(
        null=True,
        blank=True,
        verbose_name="Пробний період (днів)"
    )
    ...
    def __str__(self):
        type_display = dict(self.LICENSE_TYPE_CHOICES).get(self.license_type, self.license_type)
        if self.key:
