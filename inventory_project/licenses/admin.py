...
class LicenseAdmin(ModelAdmin):
    ...
    def license_status(self, obj):
        ...
        if obj.end_date < today:
            ...
            if days_left <= 30:
                return format_html(
                    '<span class="badge badge-warning">Закінчується</span>'
                )
            else:
    ...
    fieldsets = (
        ("Основна інформація", {"fields": ("license_type", "key", "description")}),
        ("Активації", {"fields": ("activations",)}),
        ("Дати", {"fields": ("start_date", "end_date")}),
        ("Прив'язка", {"fields": ("user",)}),
    )
