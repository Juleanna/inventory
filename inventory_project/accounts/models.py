...
class CustomUser(AbstractUser):
    ...
    position = models.CharField(
        max_length=20, choices=POSITION_CHOICES, blank=True, verbose_name="Посада"
    )
    ...
    department = models.CharField(
        max_length=20, choices=DEPARTMENT_CHOICES, blank=True, verbose_name="Відділ"
    )
    ...
    office_location = models.CharField(
        max_length=100, blank=True, verbose_name="Місцезнаходження офісу"
    )
    room_number = models.CharField(
        max_length=20, blank=True, verbose_name="Номер кімнати"
    )
    ...
    hire_date = models.DateField(
        null=True, blank=True, verbose_name="Дата прийняття на роботу"
    )
    ...
    timezone = models.CharField(
        max_length=50, default="Europe/Kiev", verbose_name="Часовий пояс"
    )
    ...
    is_temporary = models.BooleanField(
        default=False, verbose_name="Тимчасовий співробітник"
    )
    ...
    skills = models.TextField(
        blank=True, verbose_name="Навички", help_text="Професійні навички через кому"
    )

    emergency_contact_name = models.CharField(
        max_length=100, blank=True, verbose_name="Контактна особа (ім'я)"
    )
    ...
    def clean(self):
        ...
        if self.birth_date:
            ...
            if age < 16 or age > 100:
                raise ValidationError({"birth_date": _("Некоректна дата народження")})
        ...
        if self.hire_date and self.birth_date:
            if self.hire_date <= self.birth_date:
                raise ValidationError(
                    {
                        "hire_date": _(
                            "Дата прийняття на роботу не може бути раніше дати народження"
                        )
                    }
                )
    ...
    def can_manage_equipment(self, equipment):
        ...
        return (
            self.is_staff
            or equipment.current_user == self
            or equipment.responsible_person == self
            or (equipment.current_user and equipment.current_user.manager == self)
        )
...
class UserProfile(models.Model):
    ...
    dashboard_layout = models.JSONField(
        default=dict, blank=True, verbose_name="Налаштування дашборду"
    )
