...
class MaintenanceRequest(models.Model):
    ...
    request_type = models.CharField(
        max_length=20, choices=REQUEST_TYPES, verbose_name="Тип запиту"
    )
    ...
    status = models.CharField(
        max_length=15, choices=STATUS_CHOICES, default="PENDING", verbose_name="Статус"
    )
    ...
    requested_date = models.DateTimeField(
        default=timezone.now, verbose_name="Дата запиту"
    )

    scheduled_date = models.DateTimeField(
        null=True, blank=True, verbose_name="Запланована дата"
    )

    started_date = models.DateTimeField(
        null=True, blank=True, verbose_name="Дата початку робіт"
    )

    completed_date = models.DateTimeField(
        null=True, blank=True, verbose_name="Дата завершення"
    )
    ...
    downtime_required = models.BooleanField(
        default=False, verbose_name="Потрібна зупинка обладнання"
    )

    estimated_duration = models.DurationField(
        null=True, blank=True, verbose_name="Очікувана тривалість"
    )

    actual_duration = models.DurationField(
        null=True, blank=True, verbose_name="Фактична тривалість"
    )
...
class MaintenanceSchedule(models.Model):
    ...
    frequency = models.CharField(
        max_length=15, choices=FREQUENCY_CHOICES, verbose_name="Періодичність"
    )

    custom_interval_days = models.PositiveIntegerField(
        null=True, blank=True, verbose_name="Інтервал (дні)"
    )
    ...
    checklist = models.JSONField(
        default=list, blank=True, verbose_name="Чек-лист робіт"
    )
...
class MaintenanceService:
    ...
    def complete_maintenance(
        cls, request_id, technician, notes=None, actual_cost=None, parts_used=None
    ):
        """Завершити ТО"""
        try:
            request = MaintenanceRequest.objects.get(
    ...
    def get_maintenance_dashboard(cls, user=None):
        ...
        data["completed_this_week"] = base_qs.filter(
            status="COMPLETED", completed_date__gte=week_start
        ).count()

        # Мої призначення (для техніків)
        if user:
            data["my_assignments"] = base_qs.filter(
                assigned_technician=user, status__in=["APPROVED", "IN_PROGRESS"]
            ).count()

        # Майбутнє ТО
        upcoming = MaintenanceSchedule.objects.filter(
            is_active=True, next_maintenance__lte=timezone.now() + timedelta(days=30)
    ...
    def _notify_maintenance_request(cls, request):
        ...
        maintenance_managers = User.objects.filter(
            groups__name="Maintenance Managers", is_active=True
        )
