...
class SystemSerializer(serializers.ModelSerializer):
    ...
    owner_name = serializers.CharField(
        source="owner.get_full_name", read_only=True, default=""
    )
...
class SystemAccountSerializer(serializers.ModelSerializer):
    ...
    password_expired = serializers.BooleanField(
        source="is_password_expired", read_only=True
    )
        ...
        if new_password:
            ...
            if request:
                PasswordManagementService.log_password_access(
                    instance, request.user, "edit", request, "Пароль змінено через API"
                )
...
class SystemCategoryViewSet(viewsets.ModelViewSet):
    queryset = SystemCategory.objects.annotate(systems_count=Count("system")).order_by(
        "name"
    )
...
class SystemViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return (
            System.objects.annotate(accounts_count=Count("system_accounts"))
    ...
    def accounts(self, request, pk=None):
        ...
        accessible_accounts = PasswordManagementService.get_user_accessible_accounts(
            request.user
        )
        ...
        serializer = SystemAccountSerializer(
...
class SystemAccountViewSet(viewsets.ModelViewSet):
    ...
    def get_password(self, request, pk=None):
        ...
        try:
            ...
            PasswordManagementService.log_password_access(
                account, request.user, "view", request, "Пароль переглянуто через API"
            )

            return Response(
                {
                    "password": password,
                    "strength": PasswordManagementService.get_password_strength_score(
                        password
                    ),
    ...
    def statistics(self, request):
        ...
        expiring_soon = (
            PasswordManagementService.check_expiring_passwords(7)
            .filter(id__in=accessible_accounts.values_list("id", flat=True))
            .count()
        )
