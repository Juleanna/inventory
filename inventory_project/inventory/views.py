...
class EquipmentViewSet(ModelViewSet):
    ...
    def history(self, request, pk=None):
        ...
        for record in reversed(list(history_records)):
            entry = {
                ...
                "history_user": (
                    str(record.history_user) if record.history_user else None
                ),
    ...
    def documents(self, request, pk=None):
        ...
        if request.method == "GET":
            docs = EquipmentDocument.objects.filter(equipment=equipment).order_by(
                "-uploaded_at"
            )
            data = [
                {
            ...
            if not file:
                return Response(
                    {"error": "Файл не надано"}, status=status.HTTP_400_BAD_REQUEST
                )
    ...
    def bulk_update(self, request):
        ...
        if not ids or not new_status:
            return Response(
                {"error": "ids та status обовʼязкові"},
    ...
    def bulk_delete(self, request):
        ...
        if not ids:
            return Response(
                {"error": "ids обовʼязкові"}, status=status.HTTP_400_BAD_REQUEST
            )
...
def delete_equipment_document(request, equipment_id, doc_id):
    ...
    try:
        ...
        return Response(
            {"error": "Документ не знайдено"}, status=status.HTTP_404_NOT_FOUND
        )
...
class LicenseViewSet(ModelViewSet):
    queryset = (
        License.objects.select_related("user")
        .prefetch_related("licensed_software")
        .order_by("-id")
    )
...
class SoftwareViewSet(ModelViewSet):
    queryset = (
        Software.objects.select_related("license")
        .prefetch_related("installed_on")
        .all()
    )
...
def agent_report(request):
    ...
    if not serial_number:
        return Response(
            {"error": "serial_number is required"}, status=status.HTTP_400_BAD_REQUEST
        )
...
def register(request):
    ...
    if serializer.is_valid():
        ...
        return Response(
            {"message": "Користувача створено"}, status=status.HTTP_201_CREATED
        )
...
def add_equipment(request):
    ...
    if serializer.is_valid():
        ...
        return Response(
            {"message": "Оборудование добавлено", "equipment": serializer.data}
        )
    return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
def change_password(request):
    ...
    if not request.user.check_password(old_password):
        return Response(
            {"detail": "Невірний поточний пароль"}, status=status.HTTP_400_BAD_REQUEST
        )
        ...
        results.append(
            {
                ...
                "hire_date": (
                    u.hire_date.isoformat() if getattr(u, "hire_date", None) else None
                ),
                "is_staff": u.is_staff,
...
def user_create(request):
    ...
    if not request.user.is_staff:
        return Response(
            {"detail": "Недостатньо прав"}, status=status.HTTP_403_FORBIDDEN
        )
    data = request.data
    ...
    if not username or not password:
        return Response(
            {"detail": "Ім'я користувача та пароль обов'язкові"},
            status=status.HTTP_400_BAD_REQUEST,
        )
...
def users_bulk_action(request):
    ...
    if not ids:
        return Response(
            {"detail": "Не вказано жодного користувача"},
            status=status.HTTP_400_BAD_REQUEST,
        )
...
def users_stats(request):
    ...
    without_equipment = (
        ...
        .annotate(eq_count=models.Count("assigned_equipment"))
        .filter(eq_count=0)
        .count()
    )
...
def users_import(request):
    ...
    if not users_data:
        return Response(
            {"detail": "Немає даних для імпорту"}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    def get(self, request):
        ...
        try:
            if report_type == "inventory":
                ...
                return Response({"type": "inventory", "data": data, "count": len(data)})

            elif report_type == "financial":
                data = ReportService.generate_financial_report()
                return Response({"type": "financial", "data": data})

            elif report_type == "maintenance":
                data = ReportService.generate_maintenance_report()
                return Response(
                    {"type": "maintenance", "data": data, "count": len(data)}
                )

            else:
                return Response(
                    {"error": "Невідомий тип звіту"}, status=status.HTTP_400_BAD_REQUEST
                )
...
class ExportView(APIView):
    ...
    def _export_excel(self, request, report_type):
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="inventory_report_{report_type}.xlsx"'
        )
        return response
    ...
    def _create_maintenance_excel(self, workbook):
        ...
        for row, item in enumerate(data, 1):
            ...
            worksheet.write(
                row,
                3,
                str(item["last_maintenance"]) if item["last_maintenance"] else "",
            )
            worksheet.write(
                row,
                4,
                str(item["next_maintenance"]) if item["next_maintenance"] else "",
            )
    ...
    def _export_pdf(self, request, report_type):
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="inventory_report_{report_type}.pdf"'
        )
        return response
    ...
    def get(self, request):
        ...
        if data_type == "all":
            ...
            data = {
                ...
                "notifications": OfflineDataManager.get_cached_data(
        ...
        return Response(
            {
                "success": success,
                "message": (
                    "Дані кешовані для офлайн роботи"
                    if success
                    else "Помилка кешування даних"
                ),
            }
        )
...
class OfflineActionsView(APIView):
    ...
    def get(self, request):
        ...
        return Response({"success": True, "actions": actions, "count": len(actions)})
    ...
    def post(self, request):
        ...
        if not action_type:
            return Response(
                {"success": False, "error": "Не вказано тип дії"}, status=400
            )

        action_id = OfflineDataManager.store_offline_action(
            user_id, action_type, action_data
        )

        return Response({"success": True, "action_id": action_id})
    ...
    def delete(self, request):
        ...
        return Response({"success": True, "message": "Дії очищено"})
...
class OfflineSearchView(APIView):
    ...
    def get(self, request):
        ...
        if len(query) < 2:
            return Response(
                {"success": False, "error": "Запит повинен містити мінімум 2 символи"}
            )

        results = []

        if search_type in ["all", "equipment"]:
            equipment_results = OfflineSearchHelper.search_equipment(
                user_id, query, limit
            )
            results.extend(equipment_results)

        if search_type in ["all", "notifications"]:
            notification_results = OfflineSearchHelper.search_notifications(
                user_id, query, limit
            )
...
def quick_report(request):
    ...
    if not description:
        return Response(
            {"success": False, "error": "Опис проблеми обов'язковий"}, status=400
        )
...
class UserPreferencesView(APIView):
    ...
    def get(self, request):
        ...
        return Response({"success": True, "preferences": preferences_data})
    ...
    def post(self, request):
        ...
        allowed_fields = [
            ...
            "notifications_email",
            "notifications_push",
            "notifications_daily_digest",
            "notifications_maintenance",
            "notifications_warranty",
            "dashboard_shortcuts",
            "dashboard_widgets",
            "default_filters",
        ]
        ...
        filtered_data = {
            k: v for k, v in preferences_data.items() if k in allowed_fields
        }
...
class DashboardWidgetsView(APIView):
    ...
    def post(self, request):
        ...
        if not widget_type:
            return Response(
                {"success": False, "error": "Не вказано тип віджету"}, status=400
            )
        ...
        try:
            ...
            return Response(
                {"success": True, "message": "Налаштування віджету оновлено"}
            )
        except Exception as e:
            return Response({"success": False, "error": str(e)}, status=400)
...
class FavoriteEquipmentView(APIView):
    ...
    def get(self, request):
        ...
        return Response({"success": True, "favorites": list(favorite_equipment)})
    ...
    def post(self, request):
        ...
        if not equipment_id:
            return Response(
                {"success": False, "error": "Не вказано ID обладнання"}, status=400
            )
        ...
        try:
            ...
            return Response(
                {"success": False, "error": "Обладнання не знайдено"}, status=404
            )
...
class AdvancedSearchView(APIView):
    ...
    def post(self, request):
        ...
        if filters.get("purchase_date_from"):
            equipment_qs = equipment_qs.filter(
                purchase_date__gte=filters["purchase_date_from"]
            )

        if filters.get("purchase_date_to"):
            equipment_qs = equipment_qs.filter(
                purchase_date__lte=filters["purchase_date_to"]
            )
        ...
        if filters.get("price_from"):
            equipment_qs = equipment_qs.filter(
                purchase_price__gte=filters["price_from"]
            )
            ...
            results.append(
                {
                    ...
                    "purchase_price": (
                        float(eq.purchase_price) if eq.purchase_price else None
                    ),
                    "warranty_until": (
                        eq.warranty_until.isoformat() if eq.warranty_until else None
                    ),
                    "next_maintenance_date": (
                        eq.next_maintenance_date.isoformat()
...
class SearchSuggestionsView(APIView):
    ...
    def get(self, request):
        ...
        equipment_names = (
            Equipment.objects.filter(name__icontains=query)
        ...
        for name in equipment_names:
            suggestions.append(
                {"text": name, "type": "equipment_name", "category": "Назви обладнання"}
            )

        # Пошук по серійних номерах
        serial_numbers = (
            Equipment.objects.filter(serial_number__icontains=query)
        ...
        for serial in serial_numbers:
            suggestions.append(
                {"text": serial, "type": "serial_number", "category": "Серійні номери"}
            )

        # Пошук по локаціях
        locations = (
            Equipment.objects.filter(location__icontains=query)
        ...
        for location in locations:
            if location:  # Перевірити що локація не пуста
                suggestions.append(
                    {"text": location, "type": "location", "category": "Локації"}
                )

        return Response({"success": True, "suggestions": suggestions[:limit]})
...
def save_search_filter(request):
    ...
    if not filter_name or not filter_data:
        return Response(
            {"success": False, "error": "Не вказано назву або дані фільтру"}, status=400
        )
    ...
    try:
        ...
        return Response(
            {"success": True, "message": f'Фільтр "{filter_name}" збережено'}
        )
    except Exception as e:
        return Response({"success": False, "error": str(e)}, status=400)
...
def get_saved_filters(request):
    ...
    return Response({"success": True, "filters": saved_filters})
...
class TwoFactorSetupView(APIView):
    ...
    def get(self, request):
        ...
        if TwoFactorAuthService.is_2fa_enabled(user):
            return Response(
                {"success": False, "error": "Двофакторна аутентифікація вже увімкнена"},
                status=400,
            )
    ...
    def post(self, request):
        ...
        if not token:
            return Response(
                {"success": False, "error": "Не вказано код підтвердження"}, status=400
            )
        ...
        if not totp.verify(token, valid_window=1):
            return Response(
                {"success": False, "error": "Неправильний код підтвердження"},
                status=400,
            )
...
class TwoFactorDisableView(APIView):
    ...
    def post(self, request):
        ...
        if not user.check_password(password):
            return Response(
                {"success": False, "error": "Неправильний пароль"}, status=400
            )
        ...
        if not valid_token:
            return Response(
                {"success": False, "error": "Неправильний код аутентифікації"},
                status=400,
            )
        ...
        return Response(
            {"success": True, "message": "Двофакторна аутентифікація відключена"}
        )
...
class TwoFactorVerifyView(APIView):
    ...
    def post(self, request):
        ...
        if not token:
            return Response(
                {"success": False, "error": "Не вказано код аутентифікації"}, status=400
            )

        # Перевірити чи увімкнена 2FA
        if not TwoFactorAuthService.is_2fa_enabled(user):
            return Response(
                {"success": False, "error": "Двофакторна аутентифікація не увімкнена"},
                status=400,
            )
        ...
        if TwoFactorAuthService.verify_token(user, token):
            ...
            return Response(
                {"success": True, "message": "Аутентифікацію пройдено успішно"}
            )
        ...
        return Response(
            {"success": False, "error": "Неправильний код аутентифікації"}, status=400
        )
...
class TwoFactorStatusView(APIView):
    ...
    def get(self, request):
        ...
        backup_codes_count = (
            len(TwoFactorAuthService.get_backup_codes(user)) if is_enabled else 0
        )
...
def logout_2fa(request):
    ...
    return Response({"success": True, "message": "2FA сесія завершена"})
...
class MaintenanceRequestViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_staff:
            ...
            return (
                ...
                .select_related("equipment", "requester", "assigned_technician")
            ...
            results.append(
                {
                    ...
                    "requester": (
                        {
                            "id": mr.requester.id,
                            "name": mr.requester.get_full_name()
                            or mr.requester.username,
                        }
                        if mr.requester
                        else None
                    ),
                    "assigned_technician": (
                        {
                            ...
                            "name": mr.assigned_technician.get_full_name()
                            or mr.assigned_technician.username,
                        }
                        if mr.assigned_technician
                        else None
                    ),
                    "created_at": (
                        mr.requested_date.isoformat() if mr.requested_date else None
                    ),
                    "requested_date": (
                        mr.requested_date.isoformat() if mr.requested_date else None
                    ),
                    "scheduled_date": (
                        mr.scheduled_date.isoformat() if mr.scheduled_date else None
                    ),
                    "completed_date": (
                        mr.completed_date.isoformat() if mr.completed_date else None
                    ),
                    "estimated_cost": (
                        float(mr.estimated_cost) if mr.estimated_cost else None
                    ),
    ...
    def create(self, request):
        ...
        if not all([equipment_id, title, description]):
            return Response(
                {"success": False, "error": "Не вказано обов'язкові поля"}, status=400
            )
        ...
        try:
            ...
            data = {
                ...
                "assigned_technician": (
                    ...
                    if maintenance_request.assigned_technician
                    else None
                ),
                "dates": {
                    "requested": maintenance_request.requested_date.isoformat(),
                    "scheduled": (
                        maintenance_request.scheduled_date.isoformat()
                        if maintenance_request.scheduled_date
                        else None
                    ),
                    "started": (
                        maintenance_request.started_date.isoformat()
                        if maintenance_request.started_date
                        else None
                    ),
                    "completed": (
                        maintenance_request.completed_date.isoformat()
                ...
                "costs": {
                    "estimated": (
                        float(maintenance_request.estimated_cost)
                        if maintenance_request.estimated_cost
                        else None
                    ),
                    "actual": (
                        float(maintenance_request.actual_cost)
                ...
                "duration": {
                    "estimated": (
                        str(maintenance_request.estimated_duration)
                        if maintenance_request.estimated_duration
                        else None
                    ),
                    "actual": (
                        str(maintenance_request.actual_duration)
                        if maintenance_request.actual_duration
                        else None
                    ),
                },
                ...
                "tasks": [
                    {
                        ...
                        "actual_duration": (
                            str(task.actual_duration) if task.actual_duration else None
                        ),
                        "started_at": (
            ...
            return Response(
                {"success": False, "error": "Запит не знайдено"}, status=404
            )
...
class AssignTechnicianView(APIView):
    ...
    def post(self, request):
        ...
        if not request_id or not technician_id:
            return Response(
                {"success": False, "error": "Не вказано ID запиту або техніка"},
                status=400,
            )
        ...
        try:
            ...
            return Response(
                {"success": False, "error": "Техніка не знайдено"}, status=404
            )
...
class StartMaintenanceView(APIView):
    ...
    def post(self, request):
        ...
        if not request_id:
            return Response(
                {"success": False, "error": "Не вказано ID запиту"}, status=400
            )

        maintenance_request = MaintenanceService.start_maintenance(
            request_id=request_id, technician=request.user
        )

        if maintenance_request:
            return Response({"success": True, "message": "ТО розпочато"})
        else:
            return Response(
                {
                    "success": False,
...
class CompleteMaintenanceView(APIView):
    ...
    def post(self, request):
        ...
        if maintenance_request:
            return Response({"success": True, "message": "ТО завершено успішно"})
        else:
            return Response(
                {
                    "success": False,
...
class MaintenanceScheduleView(APIView):
    ...
    def get(self, request):
        ...
        schedules_qs = (
            ...
            if show_inactive
            else MaintenanceSchedule.objects.filter(is_active=True)
        )
        ...
        return Response({"success": True, "schedules": data})

    def post(self, request):
        """Створити новий розклад ТО"""
        if not request.user.is_staff:
            return Response({"success": False, "error": "Недостатньо прав"}, status=403)
...
class MaintenanceScheduleDetailView(APIView):
    ...
    def patch(self, request, pk):
        ...
        if not schedule:
            return Response(
                {"success": False, "error": "Розклад не знайдено"}, status=404
            )
        ...
        if "estimated_duration_hours" in data:
            schedule.estimated_duration = timedelta(
                hours=float(data["estimated_duration_hours"])
            )
        if "is_active" in data:
        ...
        return Response({"success": True, "message": "Розклад оновлено"})
...
def get_technicians(request):
    ...
    technicians = User.objects.filter(
        models.Q(groups__name="Technicians") | models.Q(is_staff=True), is_active=True
    ).distinct()
    ...
    data = [
        {
            ...
            "active_assignments": MaintenanceRequest.objects.filter(
                assigned_technician=tech, status__in=["APPROVED", "IN_PROGRESS"]
            ).count(),
        }
        for tech in technicians
    ]

    return Response({"success": True, "technicians": data})
...
class SparePartsViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = SparePart.objects.select_related(
            "category", "primary_supplier", "storage"
        )
    ...
    def low_stock(self, request):
        ...
        return Response({"success": True, "parts": serializer.data})
...
class SparePartMovementView(APIView):
            ...
            data.append(
                {
                    ...
                    "equipment": (
                        {"id": movement.equipment.id, "name": movement.equipment.name}
                        if movement.equipment
                        else None
                    ),
                    "notes": movement.notes,
                    "performed_by": (
                        movement.performed_by.get_full_name()
    ...
    def post(self, request):
        ...
        try:
            ...
            maintenance_request = (
                MaintenanceRequest.objects.get(id=maintenance_request_id)
            ...
            if movement_type in [
                "ISSUE",
                "WRITE_OFF",
            ] and spare_part.quantity_in_stock < int(quantity):
                return Response(
                    {"success": False, "error": "Недостатньо запчастин в наявності"},
                    status=400,
                )
            ...
            return Response(
                {"success": False, "error": "Запчастину не знайдено"}, status=404
            )
        except Equipment.DoesNotExist:
            return Response(
                {"success": False, "error": "Обладнання не знайдено"}, status=404
...
class IssueSparePartView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {"success": False, "error": "Запит на ТО не знайдено"}, status=404
            )
...
class PurchaseOrdersViewSet(ModelViewSet):
    ...
    def get_serializer_class(self):
        ...
        class PurchaseOrderSerializer(serializers.ModelSerializer):
            ...
            supplier_details = SupplierBriefSerializer(
    ...
    def receive(self, request, pk=None):
        ...
        if not received_items:
            return Response(
                {"success": False, "error": "Не вказано отримані позиції"}, status=400
            )

        purchase_order, error = SparePartsService.receive_purchase_order(
            order_id=order.id, received_items=received_items, performed_by=request.user
        )
...
def get_spare_parts_for_equipment(request, equipment_id):
    ...
    try:
        ...
        return Response({"success": True, "spare_parts": serializer.data})
...
def spare_parts_analytics(request):
    ...
    status_stats = (
        SparePart.objects.values("status")
    ...
    movement_stats = (
        SparePartMovement.objects.filter(performed_at__gte=month_ago)
    ...
    supplier_stats = (
        ...
        .annotate(parts_count=Count("primary_parts"))
        .order_by("-parts_count")[:10]
        .values("name", "parts_count", "rating")
    )
    ...
    return Response(
        {
            ...
            "analytics": {
                "overview": {
                    ...
                    "out_of_stock_count": SparePart.objects.filter(
                        status="OUT_OF_STOCK"
                    ).count(),
                },
