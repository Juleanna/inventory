...
def equipment_analytics(request):
    ...
    try:
        ...
        by_category = (
            ...
            .annotate(count=Count("id"), total_value=Sum("purchase_price"))
            .order_by("-count")
        )
        ...
        by_manufacturer = (
            Equipment.objects.exclude(manufacturer__isnull=True)
            .values("manufacturer")
            .annotate(count=Count("id"))
            .order_by("-count")[:10]
        )  # Топ 10
        ...
        by_location = (
            ...
            .annotate(
                count=Count("id"), working_count=Count("id", filter=Q(status="WORKING"))
            )
            .order_by("-count")[:10]
        )

        # Фінансова статистика
        total_value = Equipment.objects.aggregate(total=Sum("purchase_price"))[
            "total"
        ] or Decimal("0.00")

        avg_price = Equipment.objects.aggregate(avg=Avg("purchase_price"))[
            "avg"
        ] or Decimal("0.00")
        ...
        for year in range(current_year - 5, current_year + 1):
            count = Equipment.objects.filter(purchase_date__year=year).count()
            if count > 0:
                equipment_by_year.append({"year": year, "count": count})
        ...
        response_data = {
            "summary": {
                ...
                "locations_count": Equipment.objects.values("location")
                .distinct()
                .count(),
            },
...
def financial_analytics(request):
    ...
    try:
        # Загальна вартість
        total_purchase_value = Equipment.objects.aggregate(total=Sum("purchase_price"))[
            "total"
        ] or Decimal("0.00")
        ...
        equipment_with_prices = Equipment.objects.filter(
            purchase_price__isnull=False, purchase_date__isnull=False
        )
        ...
        for year in range(current_year - 5, current_year + 1):
            year_total = Equipment.objects.filter(purchase_date__year=year).aggregate(
        ...
        expensive_equipment = (
            ...
            .values("name", "purchase_price", "purchase_date", "category")
        )
        ...
        response_data = {
            "summary": {
                ...
                "depreciation_percentage": float(
                    (
                        (total_purchase_value - current_value)
    ...
    try:
        ...
        maintenance_by_category = (
            ...
            .annotate(
                ...
                maintained_count=Count(
                    "id", filter=Q(last_maintenance_date__isnull=False)
                ),
                overdue_count=Count(
                    "id",
                    filter=Q(
...
def user_analytics(request):
    ...
    try:
        ...
        users_with_equipment = (
            User.objects.filter(assigned_equipment__isnull=False).distinct().count()
        )

        # Топ користувачів по кількості обладнання
        top_users = (
            User.objects.annotate(equipment_count=Count("assigned_equipment"))
        ...
        for user in top_users:
            total_value = Equipment.objects.filter(current_user=user).aggregate(
                total=Sum("purchase_price")
            )["total"] or Decimal("0.00")
        ...
        equipment_by_department = (
            Equipment.objects.filter(current_user__isnull=False)
            .values("current_user__department")
            .annotate(count=Count("id"), total_value=Sum("purchase_price"))
            .exclude(current_user__department="")
            .order_by("-count")
        )
...
def advanced_analytics(request):
    ...
    try:
        ...
        for i in range(8):
            ...
            count = Equipment.objects.filter(
                purchase_date__gte=year_start, purchase_date__lt=year_end
            ).count()
        ...
        for i in range(24):
            ...
            qs = Equipment.objects.filter(
                purchase_date__gte=month_start, purchase_date__lte=month_end
            )
            count = qs.count()
        ...
        for cat in categories:
            ...
            for i in range(6):
                ...
                if m_start.month == 12:
                    m_end = m_start.replace(year=m_start.year + 1, month=1) - timedelta(
                        days=1
                    )
                else:
        ...
        for i in range(6):
            ...
            total = float(
                Equipment.objects.filter(
                    purchase_date__gte=m_start, purchase_date__lte=m_end
                ).aggregate(total=Sum("purchase_price"))["total"]
                or 0
            )
        ...
        return Response(
            {
                ...
                "summary": {
                    ...
                    "total_active_equipment": Equipment.objects.filter(
                        status="WORKING"
                    ).count(),
    ...
    try:
        ...
        if report_type == "financial":
            ...
            report_data = {
                ...
                "total_value": float(
                    equipment_queryset.aggregate(total=Sum("purchase_price"))["total"]
                    or Decimal("0.00")
                ),
                "by_category": list(
                    equipment_queryset.values("category")
                    .annotate(count=Count("id"), total_value=Sum("purchase_price"))
                    .order_by("-total_value")
                ),
                "expensive_items": list(
                    equipment_queryset.filter(purchase_price__isnull=False)
                    .order_by("-purchase_price")[:20]
                    .values("name", "serial_number", "purchase_price", "purchase_date")
                ),
            }
            ...
            report_data = {
                ...
                "maintenance_schedule": list(
                    equipment_queryset.filter(next_maintenance_date__isnull=False)
            ...
            report_data = {
                ...
                "summary": {
                    "total_equipment": equipment_queryset.count(),
                    "total_value": float(
                        equipment_queryset.aggregate(total=Sum("purchase_price"))[
                            "total"
                        ]
                        or Decimal("0.00")
                    ),
                    "by_status": list(
                        equipment_queryset.values("status")
                        .annotate(count=Count("id"))
                        .order_by("-count")
                    ),
                    "by_category": list(
                    ...
                    "by_location": list(
                        equipment_queryset.values("location")
                        .annotate(count=Count("id"))
                        .order_by("-count")[:10]
                    ),
                },
