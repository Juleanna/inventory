...
class EquipmentTemplate(models.Model):
    ...
    description = models.TextField(blank=True, default='')
    category = models.CharField(max_length=20, choices=Equipment.CATEGORY_CHOICES, default='PC')
    ...
    created_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, related_name='equipment_templates')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class ActivityLogSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'target_object_id', 'target_model', 'target_name',
...
class EquipmentTemplateViewSet(viewsets.ModelViewSet):
    ...
    def create_equipment(self, request, pk=None):
        ...
        # Apply overrides
        for key in ['serial_number', 'inventory_number', 'purchase_price', 'purchase_date',
                     'warranty_until', 'notes', 'description']:
            if key in overrides and overrides[key]:
                equipment_data[key] = overrides[key]

        try:
            eq = Equipment.objects.create(**equipment_data)
            return Response({'id': eq.id, 'name': eq.name}, status=status.HTTP_201_CREATED)
...
class GlobalSearchView(APIView):
    ...
    def get(self, request):
        ...
        equipment_qs = Equipment.objects.filter(
            ...
            Q(inventory_number__icontains=q) | Q(model__icontains=q) |
        ...
        if users_qs.exists():
            results['users'] = [
                {'type': 'user', 'id': u.id, 'title': u.get_full_name() or u.username,
                 'subtitle': u.email,
                 'url': f'/users/{u.id}'}
                for u in users_qs
            ]
    ...
    def get(self, request):
        ...
        for item in items:
            ...
            if cat not in by_category:
                by_category[cat] = {'category': cat, 'purchase_value': 0, 'book_value': 0, 'depreciation': 0}
        ...
        for eq in equipment:
            ...
            if loc not in by_location:
                by_location[loc] = {'location': loc, 'purchase_value': 0, 'book_value': 0, 'count': 0}
...
class EquipmentCompareView(APIView):
    ...
    def post(self, request):
        ...
        if not ids or len(ids) < 2:
            return Response({'error': 'Потрібно вибрати щонайменше 2 одиниці'}, status=400)
        ...
        for eq in equipment:
            result.append({
                ...
                'purchase_price': float(eq.purchase_price) if eq.purchase_price else 0,
                'warranty_until': eq.warranty_until.isoformat() if eq.warranty_until else '—',
                'current_user': eq.current_user.get_full_name() if eq.current_user else '—',
            })
        ...
        for eq in equipment:
            ...
            if room not in locations[building][floor]:
                locations[building][floor][room] = {
                    'total': 0, 'working': 0, 'repair': 0, 'items': []
                }
...
class EmailSettingsView(APIView):
    ...
    def put(self, request):
        ...
        try:
            ...
            return Response({'status': 'ok', 'message': f'Тестовий лист надіслано на {to_email}'})
        except Exception as e:
...
class LdapSettingsView(APIView):
    ...
    def put(self, request):
        ...
        if not server_url:
            return Response({'status': 'error', 'message': 'URL сервера не вказано'}, status=400)
        try:
            import ldap
            ...
            return Response({'status': 'error', 'message': 'python-ldap не встановлено'}, status=400)
        except Exception as e:
            return Response({'status': 'error', 'message': str(e)}, status=400)
...
class DashboardWidgetsView(APIView):
    ...
    def get(self, request):
        ...
        widgets = prefs.dashboard_widgets or [
            {'id': 'equipment_overview', 'label': 'Огляд обладнання', 'visible': True, 'order': 0},
            {'id': 'financial_overview', 'label': 'Фінансовий огляд', 'visible': True, 'order': 1},
            {'id': 'maintenance_alerts', 'label': 'Сповіщення обслуговування', 'visible': True, 'order': 2},
            {'id': 'recent_activity', 'label': 'Остання активність', 'visible': True, 'order': 3},
            {'id': 'equipment_by_status', 'label': 'Обладнання за статусом', 'visible': True, 'order': 4},
            {'id': 'equipment_by_location', 'label': 'Обладнання за локацією', 'visible': True, 'order': 5},
        ]
...
class ExportReportView(APIView):
    ...
    def _depreciation_excel(self, rows):
        ...
        headers = ['Назва', 'Категорія', 'Дата покупки', 'Вартість', 'Норма %',
                    'Вік (р.)', 'Амортизація', 'Залишкова', 'Аморт./міс']
        ...
        response['Content-Disposition'] = 'attachment; filename="depreciation_report.xlsx"'
        return response
    ...
    def _depreciation_pdf(self, rows):
        ...
        data = [['Назва', 'Категорія', 'Вартість', 'Норма %', 'Вік', 'Амортизація', 'Залишкова']]
        for row in rows:
            data.append([
                row['name'][:30], row['category'],
                f"{row['purchase_price']:.2f}", f"{row['rate']:.0f}%",
                f"{row['age']:.1f}", f"{row['depreciation']:.2f}", f"{row['book_value']:.2f}",
            ])
        ...
        response['Content-Disposition'] = 'attachment; filename="depreciation_report.pdf"'
        return response
    ...
    def post(self, request):
        ...
        if action_type == 'change_status':
            ...
            if user_id:
                ...
                return Response({'message': f'Користувача призначено для {count} одиниць'})
...
class CsvImportView(APIView):
    ...
    def post(self, request):
        ...
        for i, row in enumerate(reader, 2):
            try:
                Equipment.objects.create(
                    ...
                    serial_number=row.get('serial_number', row.get('Серійний номер', '')),
        ...
        return Response({
            'created': created,
            'errors': errors[:20],
            'message': f'Імпортовано {created} одиниць' + (f', помилок: {len(errors)}' if errors else ''),
        })
