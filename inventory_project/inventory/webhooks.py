...
class WebhookConfig(models.Model):
    ...
    secret = models.CharField(max_length=255, blank=True, default='', verbose_name='Секретний ключ')
...
class WebhookService:
    ...
    SUPPORTED_EVENTS = [
        ...
        'contract.expiring', 'license.expiring',
    ]
    ...
    def _deliver(webhook: WebhookConfig, event: str, payload: dict):
        ...
        if 'hooks.slack.com' in webhook.url:
            body = json.dumps(WebhookService._format_slack(event, payload), ensure_ascii=False, default=str)
        elif 'webhook.office.com' in webhook.url or 'microsoft.com' in webhook.url:
            body = json.dumps(WebhookService._format_teams(event, payload), ensure_ascii=False, default=str)
    ...
    def _format_slack(event: str, payload: dict) -> dict:
        ...
        message = payload.get('message', json.dumps(payload, ensure_ascii=False, default=str)[:500])
        return {
            'blocks': [
