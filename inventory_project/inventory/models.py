...
class EquipmentManager(models.Manager):
    ...
    def total_value(self):
        ...
        return self.aggregate(total=models.Sum("purchase_price"))["total"] or Decimal(
            "0.00"
        )
...
class Equipment(models.Model):
    ...
    category = models.CharField(
        max_length=10, choices=CATEGORY_CHOICES, verbose_name="Категорія", db_index=True
    )
    model = models.CharField(
        max_length=255, null=True, blank=True, verbose_name="Модель пристрою"
    )
    manufacturer = models.CharField(
        max_length=255, null=True, blank=True, verbose_name="Виробник", db_index=True
    )
    ...
    hostname = models.CharField(
        max_length=255, null=True, blank=True, verbose_name="Ім'я хоста"
    )
    ...
    last_maintenance_date = models.DateField(
        null=True, blank=True, verbose_name="Дата останнього обслуговування"
    )
    next_maintenance_date = models.DateField(
        null=True, blank=True, verbose_name="Наступне обслуговування"
    )
    expiry_date = models.DateField(
        null=True, blank=True, verbose_name="Дата закінчення строку служби"
    )
    ...
    building = models.CharField(
        max_length=100, null=True, blank=True, verbose_name="Будівля"
    )
    floor = models.CharField(
        max_length=10, null=True, blank=True, verbose_name="Поверх"
    )
    room = models.CharField(
        max_length=50, null=True, blank=True, verbose_name="Кімната"
    )
    ...
    supplier = models.CharField(
        max_length=100, null=True, blank=True, verbose_name="Постачальник"
    )
    ...
    cpu = models.CharField(
        max_length=200, null=True, blank=True, verbose_name="Процесор"
    )
    ram = models.CharField(
        max_length=100, null=True, blank=True, verbose_name="Оперативна пам'ять"
    )
    storage = models.CharField(
        max_length=200, null=True, blank=True, verbose_name="Накопичувач"
    )
    gpu = models.CharField(
        max_length=200, null=True, blank=True, verbose_name="Відеокарта"
    )
    operating_system = models.CharField(
        max_length=100, null=True, blank=True, verbose_name="Операційна система"
    )

    # Деталізовані характеристики
    motherboard = models.CharField(
        max_length=200, blank=True, verbose_name="Материнська плата"
    )
    motherboard_serial = models.CharField(
        max_length=100, blank=True, verbose_name="Серійний номер материнської плати"
    )
    disk_model = models.TextField(
        blank=True, verbose_name="Накопичувачі (деталізовано)"
    )
    display = models.CharField(
        max_length=200, blank=True, verbose_name="Екран / Монітор"
    )
    network_adapter = models.CharField(
        max_length=200, blank=True, verbose_name="Мережевий адаптер"
    )
    power_supply = models.CharField(
        max_length=200, blank=True, verbose_name="Блок живлення"
    )
    bios_version = models.CharField(
        max_length=100, blank=True, verbose_name="Версія BIOS/UEFI"
    )

    # Додаткова інформація
    description = models.TextField(
        blank=True, verbose_name="Опис", help_text="Додаткова інформація про обладнання"
    )
    notes = models.TextField(
        blank=True, verbose_name="Примітки", help_text="Внутрішні примітки"
    )

    # Зображення та коди
    barcode_image = models.ImageField(
        upload_to="barcodes/", blank=True, null=True, verbose_name="Штрих-код"
    )
    qrcode_image = models.ImageField(
        upload_to="qrcodes/", blank=True, null=True, verbose_name="QR-код"
    )
    ...
    def clean(self):
        ...
        if self.expiry_date and self.purchase_date:
            if self.expiry_date <= self.purchase_date:
                raise ValidationError(
                    {
                        "expiry_date": _(
                            "Дата закінчення не може бути раніше дати покупки"
                        )
                    }
                )
        ...
        if self.warranty_until and self.purchase_date:
            if self.warranty_until < self.purchase_date:
                raise ValidationError(
                    {
                        "warranty_until": _(
                            "Дата закінчення гарантії не може бути раніше дати покупки"
                        )
                    }
                )
        ...
        if self.next_maintenance_date and self.last_maintenance_date:
            if self.next_maintenance_date <= self.last_maintenance_date:
                raise ValidationError(
                    {
                        "next_maintenance_date": _(
                            "Дата наступного обслуговування повинна бути пізніше останнього"
                        )
                    }
                )
...
class Notification(models.Model):
    ...
    notification_type = models.CharField(
        max_length=10, choices=TYPE_CHOICES, default="INFO", verbose_name="Тип"
    )
    priority = models.CharField(
        max_length=10,
        choices=PRIORITY_CHOICES,
    ...
    expires_at = models.DateTimeField(
        null=True, blank=True, verbose_name="Закінчується"
    )
...
class Software(models.Model):
    ...
    installed_on = models.ManyToManyField(
        Equipment, blank=True, verbose_name="Встановлено на"
    )
...
class PeripheralDevice(models.Model):
    ...
    serial_number = models.CharField(
        max_length=255, unique=True, verbose_name="Серійний номер"
    )
    inventory_number = models.CharField(
        max_length=100,
        unique=True,
...
class EquipmentDocument(models.Model):
    ...
    file = models.FileField(
        upload_to="documents/%Y/%m/%d/", verbose_name="Файл документа"
    )
...
class UserPreferences(models.Model):
    ...
    theme = models.CharField(
        max_length=10, choices=THEME_CHOICES, default="light", verbose_name="Тема"
    )
    ...
    language = models.CharField(
        max_length=5, choices=LANGUAGE_CHOICES, default="uk", verbose_name="Мова"
    )
    ...
    notifications_push = models.BooleanField(
        default=True, verbose_name="Push сповіщення"
    )

    notifications_daily_digest = models.BooleanField(
        default=False, verbose_name="Щоденний дайджест"
    )

    notifications_maintenance = models.BooleanField(
        default=True, verbose_name="Сповіщення про ТО"
    )

    notifications_warranty = models.BooleanField(
        default=True, verbose_name="Сповіщення про гарантію"
    )

    # Персоналізація дашборду
    dashboard_shortcuts = models.JSONField(
        default=dict, blank=True, verbose_name="Ярлики дашборду"
    )

    dashboard_widgets = models.JSONField(
        default=list, blank=True, verbose_name="Віджети дашборду"
    )
    ...
    default_filters = models.JSONField(
        default=dict, blank=True, verbose_name="Фільтри за замовчуванням"
    )
...
class UserActivity(models.Model):
    ...
    action_type = models.CharField(
        max_length=20, choices=ACTION_TYPES, verbose_name="Тип дії"
    )

    target_object_id = models.PositiveIntegerField(
        null=True, blank=True, verbose_name="ID об'єкта"
    )

    target_model = models.CharField(
        max_length=50, null=True, blank=True, verbose_name="Модель об'єкта"
    )
...
class CustomDashboard(models.Model):
    ...
    is_default = models.BooleanField(
        default=False, verbose_name="Дашборд за замовчуванням"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.is_default:
            CustomDashboard.objects.filter(user=self.user, is_default=True).exclude(
                pk=self.pk
