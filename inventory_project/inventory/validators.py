...
def validate_serial_number(value):
    ...
    if not re.match(r"^[A-Za-z0-9\-_]+$", value):
        raise ValidationError(
            _(
                "Серійний номер може містити тільки літери, цифри, дефіси та підкреслення"
            )
        )
...
def validate_mac_address(value):
    ...
    if not any(re.match(pattern, value) for pattern in patterns):
        raise ValidationError(
            _("Невірний формат MAC-адреси. Використовуйте формат: 00:11:22:33:44:55")
        )
...
def validate_phone_number(value):
    ...
    if not re.match(pattern, value):
        raise ValidationError(
            _(
                "Невірний формат номера телефону. Використовуйте формат: +380501234567 або 0501234567"
            )
        )
