...
class BackupListView(APIView):
    ...
    def get(self, request):
        ...
        gdrive_authorized = backup_service.is_gdrive_authorized() if gdrive_configured else False
        gdrive_backups = []
...
class BackupDownloadView(APIView):
    ...
    def get(self, request, filename):
        if not filename.startswith('inventory_backup_') or '..' in filename:
            return Response({'error': 'Невалідне ім\'я файлу'}, status=status.HTTP_400_BAD_REQUEST)
...
class BackupUploadGDriveView(APIView):
    ...
    def post(self, request):
        ...
        if not filename:
            return Response({'error': 'Вкажіть filename'}, status=status.HTTP_400_BAD_REQUEST)
...
class GDriveAuthorizeView(APIView):
    ...
    def post(self, request):
        ...
        if not code:
            return Response({'error': 'Вкажіть код авторизації'}, status=status.HTTP_400_BAD_REQUEST)
...
class GDriveUploadCredentialsView(APIView):
    ...
    def post(self, request):
        ...
        if not file:
            return Response({'error': 'Завантажте файл credentials'}, status=status.HTTP_400_BAD_REQUEST)
...
class BackupSettingsView(APIView):
    ...
    def put(self, request):
        ...
        for key in ['auto_backup', 'auto_upload_gdrive', 'interval_hours', 'max_local_backups', 'max_age_days']:
