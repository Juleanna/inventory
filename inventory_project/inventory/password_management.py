...
class PasswordEncryptionService:
    ...
    def _get_encryption_key():
        ...
        if not password_key:
            ...
            logger.warning(
                "Згенеровано новий ключ шифрування. Збережіть його в settings.PASSWORD_ENCRYPTION_KEY"
            )
...
class System(models.Model):
    ...
    system_type = models.CharField(
        max_length=20, choices=SYSTEM_TYPES, default="web", verbose_name="Тип системи"
    )
    ...
    port = models.PositiveIntegerField(null=True, blank=True, verbose_name="Порт")
...
class SystemAccount(models.Model):
    ...
    _encrypted_password = models.TextField(
        blank=True, verbose_name="Зашифрований пароль"
    )
    ...
    password_created = models.DateTimeField(
        auto_now_add=True, verbose_name="Пароль створено"
    )

    password_expires = models.DateTimeField(
        null=True, blank=True, verbose_name="Пароль діє до"
    )

    last_password_change = models.DateTimeField(
        null=True, blank=True, verbose_name="Остання зміна пароля"
    )
    ...
    def set_password(self, password):
        ...
        logger.info(
            f"Пароль змінено для облікового запису {self.username}@{self.system.name}"
        )
    ...
    def clean(self):
        ...
        if (
            SystemAccount.objects.filter(system=self.system, username=self.username)
            .exclude(pk=self.pk)
            .exists()
        ):
        ...
        if self.password_expires and self.password_created:
            if self.password_expires <= self.password_created:
                raise ValidationError(
                    {
                        "password_expires": _(
                            "Дата закінчення дії повинна бути пізніше дати створення"
                        )
                    }
                )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.pk:
            ...
            logger.info(
                f"Створено новий обліковий запис {self.username} для системи {self.system.name}"
            )
...
class PasswordAccessLog(models.Model):
    ...
    action = models.CharField(max_length=20, choices=ACTION_TYPES, verbose_name="Дія")
...
class PasswordManagementService:
    ...
    def log_password_access(account, user, action, request=None, notes=""):
        ...
        logger.info(
            f"Зафіксовано доступ до пароля: {user.username} - {action} - {account}"
        )
