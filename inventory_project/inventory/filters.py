...
class EquipmentFilter(FilterSet):
    ...
    purchase_date_after = DateFilter(
        field_name="purchase_date", lookup_expr="gte", label="Дата покупки (с)"
    )
    purchase_date_before = DateFilter(
        field_name="purchase_date", lookup_expr="lte", label="Дата покупки (по)"
    )
