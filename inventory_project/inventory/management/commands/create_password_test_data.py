...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS("Створення тестових даних для управління паролями...")
        )
        ...
        if not users:
            self.stdout.write(
                self.style.ERROR(
                    "Немає користувачів у базі даних. Створіть спочатку користувачів."
                )
            )
            return
        ...
        for sys_data in systems_data:
            system, created = System.objects.get_or_create(
                name=sys_data["name"], defaults=sys_data
            )
        ...
        for acc_data in accounts_data:
            ...
            if created:
                ...
                self.stdout.write(
                    f"Створено обліковий запис: {account.username}@{account.system.name}"
                )
