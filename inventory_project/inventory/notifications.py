...
class NotificationService:
    ...
    def check_warranty_expiration():
        ...
        for equipment in expiring_equipment:
            ...
            it_managers = CustomUser.objects.filter(department="IT", is_staff=True)
        ...
        logger.info(
            f"Створено {notifications_created} сповіщень про закінчення гарантії"
        )
    ...
    def check_maintenance_due():
        ...
        for equipment in equipment_needing_maintenance:
            ...
            if equipment.last_maintenance_date:
                ...
                days_overdue = (
                    (today - equipment.purchase_date).days
            ...
            for user in users_to_notify:
                ...
                if not recent_notification:
                    ...
                    priority = (
                        "URGENT"
                        if days_overdue > 90
                        else "HIGH" if days_overdue > 30 else "MEDIUM"
                    )
        ...
        logger.info(
            f"Створено {notifications_created} сповіщень про технічне обслуговування"
        )
        return notifications_created
    ...
    def check_equipment_age():
        ...
        for equipment in Equipment.objects.filter(
            status="WORKING", purchase_date__isnull=False
        ):
        ...
        for equipment, age in old_equipment:
            ...
            it_managers = CustomUser.objects.filter(
                department="IT", position__in=["MANAGER", "DIRECTOR", "ADMIN"]
            )
            users_to_notify.extend(it_managers)
    ...
    def send_daily_digest(user_id=None):
        ...
        if user_id:
            ...
            users = CustomUser.objects.filter(
                notification_preferences__daily_reports=True, is_active=True
            )
        ...
        for user in users:
            ...
            unread_notifications = Notification.objects.filter(
                user=user, read=False, created_at__date=today
            ).count()

            # Відправляємо дайджест тільки якщо є що розповісти
            if (
                needs_maintenance > 0
                or warranty_expiring > 0
                or unread_notifications > 0
            ):

                subject = (
                    f"Щоденний дайджест інвентаризації - {today.strftime('%d.%m.%Y')}"
                )
                ...
                try:
                    ...
                    digests_sent += 1
                    logger.info(
                        f"Щоденний дайджест відправлено користувачу {user.username}"
                    )

                except Exception as e:
                    logger.error(
                        f"Помилка відправки дайджесту користувачу {user.username}: {e}"
                    )
