    ...
    try:
        ...
        for item in equipment_data:
            ...
            if item["qrcode_image"]:
                item["qrcode_url"] = request.build_absolute_uri(
                    equipment_obj.qrcode_image.url
                )
            else:
                item["qrcode_url"] = None

        return Response({"count": len(equipment_data), "results": list(equipment_data)})
...
def scan_qr_code(request):
    ...
    try:
        ...
        logger.info(
            f"QR-код відсканований користувачем {request.user.username} для обладнання {equipment.serial_number}"
        )
        ...
        if (
            ...
            and equipment.responsible_person != request.user
        ):
...
def update_equipment_location(request):
    ...
    try:
        ...
        try:
            ...
            return Response(
                {"error": "Обладнання не знайдено"}, status=status.HTTP_404_NOT_FOUND
            )

        # Перевірити права (тільки поточний користувач або відповідальна особа)
        if (
            equipment.current_user != request.user
            and equipment.responsible_person != request.user
        ):
...
def report_issue(request):
    ...
    try:
        ...
        issue_type = request.data.get(
            "type", "GENERAL"
        )  # GENERAL, HARDWARE, SOFTWARE, NETWORK
...
def mobile_user_equipment(request):
    ...
    try:
        user_equipment = Equipment.objects.filter(current_user=request.user).values(
            "id",
            "name",
            "category",
            "status",
            ...
            equipment_data.update(
                {
                    ...
                    "qrcode_url": (
                        request.build_absolute_uri(equipment_obj.qrcode_image.url)
                        if equipment_obj.qrcode_image
                        else None
                    ),
                }
            )
        ...
        return Response({"count": len(equipment_list), "equipment": equipment_list})
...
def mark_notification_read(request, notification_id):
    ...
    try:
        ...
        return Response({"success": True, "read_at": notification.read_at.isoformat()})

    except Notification.DoesNotExist:
        return Response(
            {"error": "Уведомлення не знайдено"}, status=status.HTTP_404_NOT_FOUND
        )
    except Exception as e:
...
def mobile_dashboard(request):
    ...
    try:
        ...
        responsible_equipment_count = Equipment.objects.filter(
        ...
        needs_attention = {
            ...
            "needs_maintenance": len(
                [eq for eq in user_equipment if eq.needs_maintenance()]
            ),
        ...
        recent_notifications = Notification.objects.filter(
            user=user, created_at__gte=today - timedelta(days=7)
        ).count()
