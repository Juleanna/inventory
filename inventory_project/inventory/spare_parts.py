...
class Supplier(models.Model):
    ...
    name = models.CharField(max_length=200, verbose_name="Повна назва постачальника")
    ...
    tax_id = models.CharField(
        max_length=50, blank=True, verbose_name="Податковий номер"
    )
...
class SparePart(models.Model):
    ...
    quantity_in_stock = models.PositiveIntegerField(
        default=0, verbose_name="Кількість в наявності"
    )

    minimum_stock_level = models.PositiveIntegerField(
        default=1, verbose_name="Мінімальний рівень запасів"
    )

    maximum_stock_level = models.PositiveIntegerField(
        default=100, verbose_name="Максимальний рівень запасів"
    )

    reorder_point = models.PositiveIntegerField(
        default=5, verbose_name="Точка перезамовлення"
    )
    ...
    weight = models.DecimalField(
        max_digits=8, decimal_places=3, null=True, blank=True, verbose_name="Вага (кг)"
    )

    dimensions = models.CharField(max_length=100, blank=True, verbose_name="Розміри")

    condition = models.CharField(
        max_length=20, choices=CONDITION_CHOICES, default="NEW", verbose_name="Стан"
    )

    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES, default="IN_STOCK", verbose_name="Статус"
    )
    ...
    storage_location = models.CharField(
        max_length=100, blank=True, verbose_name="Місце зберігання (стара)"
    )
    ...
    last_received_date = models.DateTimeField(
        null=True, blank=True, verbose_name="Дата останнього отримання"
    )

    last_issued_date = models.DateTimeField(
        null=True, blank=True, verbose_name="Дата останньої видачі"
    )

    expiry_date = models.DateField(
        null=True, blank=True, verbose_name="Термін придатності"
    )

    warranty_period_days = models.PositiveIntegerField(
        default=0, verbose_name="Гарантійний період (дні)"
    )

    # Додаткові поля
    barcode = models.CharField(max_length=100, blank=True, verbose_name="Штрих-код")

    image = models.ImageField(
        upload_to="spare_parts/", blank=True, null=True, verbose_name="Зображення"
    )
...
class SparePartMovement(models.Model):
    ...
    movement_type = models.CharField(
        max_length=20, choices=MOVEMENT_TYPES, verbose_name="Тип руху"
    )
    ...
    reference_number = models.CharField(
        max_length=100, blank=True, verbose_name="Номер документу"
    )
    ...
    performed_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, verbose_name="Виконав"
    )

    performed_at = models.DateTimeField(
        default=timezone.now, verbose_name="Дата операції"
    )
...
class PurchaseOrder(models.Model):
    ...
    order_date = models.DateTimeField(
        default=timezone.now, verbose_name="Дата замовлення"
    )

    expected_delivery_date = models.DateField(
        null=True, blank=True, verbose_name="Очікувана дата доставки"
    )

    actual_delivery_date = models.DateField(
        null=True, blank=True, verbose_name="Фактична дата доставки"
    )
    ...
    tracking_number = models.CharField(
        max_length=100, blank=True, default="", verbose_name="ТТН / Трекінг номер"
    )
...
class PurchaseOrderItem(models.Model):
    ...
    quantity_received = models.PositiveIntegerField(
        default=0, verbose_name="Отримана кількість"
    )

    unit_price = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name="Ціна одиниці"
    )

    total_price = models.DecimalField(
        max_digits=12, decimal_places=2, verbose_name="Загальна ціна"
    )
...
class SparePartsService:
    ...
    def create_spare_part(cls, name, part_number, category=None, **kwargs):
        ...
        spare_part = SparePart.objects.create(
            name=name, part_number=part_number, category=category, **kwargs
        )
        return spare_part
    ...
    def receive_purchase_order(cls, order_id, received_items, performed_by=None):
        ...
        try:
            ...
            for item_data in received_items:
                order_item = PurchaseOrderItem.objects.get(id=item_data["item_id"])
    ...
    def get_inventory_dashboard(cls):
        ...
        data["out_of_stock_count"] = SparePart.objects.filter(
            status="OUT_OF_STOCK"
        ).count()
    ...
    def _update_purchase_order_status(cls, purchase_order):
        ...
        partially_received = items.filter(
            quantity_received__gt=0, quantity_received__lt=models.F("quantity_ordered")
        ).count()
